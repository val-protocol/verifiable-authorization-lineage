...
test('v2 ASSIGNMENT with conformant carrier => authority green', async () => {
  const a = await mkRow(1, null, 'assign', assignment());
  const r = await verifyValChain([a], { delegatorAuthorityPolicy: POLICY });
...
test('v2 ASSIGNMENT without the carrier => authority red (required as of v2)', async () => {
  const a = await mkRow(1, null, 'assign', assignment({ human_attestation: { method: 'session' } }));
  const r = await verifyValChain([a]);
  assert.equal(r.authority, 'red');
  assert.match(r.firstAuthorityViolation.reason, /required as of v2/);
});

test('scope.act exceeding the delegator capability => authority red (escalation)', async () => {
  const a = await mkRow(1, null, 'assign', assignment({
...
test('unknown delegator capability => authority red (not evaluable)', async () => {
  const a = await mkRow(1, null, 'assign', assignment({
    human_attestation: {
      method: 'session',
...
test('v1 ASSIGNMENT without the carrier => legacy tolerated + counted, authority stays none', async () => {
  const a = await mkRow(1, null, 'assign', assignment({ v: 1, human_attestation: { method: 'session' } }));
  const r = await verifyValChain([a]);
...
test('no policy supplied => presence still enforced, subset check skipped', async () => {
...
test('container_owner: owner-performed send => authority green (ownership re-derived)', async () => {
  const a = await mkRow(1, null, 'assign', coAssignment());
  const b = await mkRow(2, a.chain_hash, 'share.created', communication(a.chain_hash, `user:${OWNER_ID}`));
  const r = await verifyValChain([a, b], { delegatorAuthorityPolicy: CO_POLICY });
  assert.equal(r.authority, 'green');
  assert.equal(r.firstAuthorityViolation, null);
...
test('container_owner: user principal != attested owner => authority red (masquerade caught from chain bytes)', async () => {
  const a = await mkRow(1, null, 'assign', coAssignment());
  const b = await mkRow(2, a.chain_hash, 'share.created', communication(a.chain_hash, 'user:intruder-9'));
  const r = await verifyValChain([a, b], { delegatorAuthorityPolicy: CO_POLICY });
  assert.equal(r.authority, 'red');
  assert.match(r.firstAuthorityViolation.reason, /not the attested container owner/);
});

test('container_owner: scope_ref != scope.res.in_workspace => authority red, even without a policy', async () => {
  const a = await mkRow(1, null, 'assign', coAssignment({
    human_attestation: {
      method: 'session',
      subject_user_hash: OWNER_HASH,
  ...
  const r = await verifyValChain([a]); // no policy: the consistency check is chain-internal
  assert.equal(r.authority, 'red');
  assert.match(r.firstAuthorityViolation.reason, /scope_ref/);
});

test('container_owner: agent principal => no hash cross-check (Profile-A residual), authority green', async () => {
  const a = await mkRow(1, null, 'assign', coAssignment());
  const b = await mkRow(2, a.chain_hash, 'share.created', communication(a.chain_hash, 'agent:sa-1'));
  const r = await verifyValChain([a, b], { delegatorAuthorityPolicy: CO_POLICY });
  assert.equal(r.authority, 'green');
  assert.equal(r.firstAuthorityViolation, null);
});

test('options param is additive — legacy call shape still verifies passes 1-3', async () => {
  const a = await mkRow(1, null, 'assign', assignment({ v: 1, human_attestation: { method: 'session' } }));
  const b = await mkRow(2, a.chain_hash, 'access', {
