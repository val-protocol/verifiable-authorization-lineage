...
test('LEG 1: JS validator eats real DSS-emitted bytes — parse + sig-value verify + x5c path succeed', { skip }, async () => {
  ...
  // Honest outcome: SSL.com US cert lacks QcStatements → conclusive not_qualified (NOT a false green/red).
...
test('LEG 3a: tamper the DSS signature value → SIG_CRYPTO_FAILURE', { skip }, async () => {
  ...
  const r = await validateQes({ signedCanonical: canonical, signature: { alg: 'eidas_qes', signature: tampered }, validationTime: header.iat ? new Date(header.iat * 1000).toISOString() : header.sigT, trust: { trustAnchorsDer: [anchor] } });
...
test('crit enforcement (RFC 7515): DSS crit:["sigD"] accepted; an unknown crit param is rejected', { skip }, async () => {
  ...
  const ok = await validateQes({ signedCanonical: canonical, signature: { alg: 'eidas_qes', signature: jadesB64 }, validationTime: vt, trust: { trustAnchorsDer: [anchor] } });
  ...
  const r = await validateQes({ signedCanonical: canonical, signature: { alg: 'eidas_qes', signature: evil }, validationTime: vt, trust: { trustAnchorsDer: [anchor] } });
...
test('LEG 3b: tamper the signed canonical (detached "payload") → HASH_FAILURE (binding rejects)', { skip }, async () => {
  ...
  const r = await validateQes({ signedCanonical: tamperedCanonical, signature: { alg: 'eidas_qes', signature: jadesB64 }, validationTime: header.iat ? new Date(header.iat * 1000).toISOString() : header.sigT, trust: { trustAnchorsDer: [anchor] } });
