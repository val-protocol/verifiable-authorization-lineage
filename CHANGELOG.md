...
- **`@val-protocol/chain-verifier` 0.11.0** — personal-scope self-attestation + consent-bond legibility:
  ...
  - **Qualified CONSENT fix** — a Profile-C consent signature no longer goes red as "unsupported alg": with a matching per-signature `qesValidation` verdict ⇒ verified (green); without ⇒ classified, not verified (same discipline as qualified delegations, §7.1(f) per-signature matching).
...
- **`@val-protocol/chain-verifier` 0.10.0** — implements the 2026-07-01 spec amendments (report legibility, one release):
  ...
  - **`key_binding` gains `unattested`** (spec §5.2 amendment 2026-07-02): `ValKeyBinding` widens to `device_bound | syncable | unattested`; new outcome `authority_verified_org_root_unattested`. An `unattested` binding (no verified hardware attestation at enrollment — provenance is the enrollee's claim) still earns Profile B on a verified+linked signature; the binding is the orthogonal hardware axis, surfaced verbatim, never rounded up. Additive for existing chains (`device_bound`/`syncable` blocks report unchanged); enables producers to stop over-labeling attestation-less enrollments as `device_bound` (spec §5.2 amendment 2026-07-02).
